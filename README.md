...
- **YOLO 自动标注**：使用训练好的 YOLO 模型一键自动标注，自动匹配项目标签
- **LocateAnything 自动标注**：集成 NVIDIA LocateAnything-3B 大模型，无需训练，直接基于标签名称描述检测目标，支持零样本检测；模型按需加载，标注完成后可手动卸载释放 GPU 显存
...
> ⚠️ **重要提示：升级部署后，请先清除浏览器缓存，再重新打开平台。**
>
> 否则浏览器可能继续使用旧版本的 JavaScript 和 CSS 文件，导致页面功能或样式异常。
...
| 日期 | 更新内容                                               |
|:---|:---------------------------------------------------|
| `2026-08-24` | 优化任务与审核页面卡片布局；修复训练停止和标注保存等问题；新增模型分组、按项目应用持久化、模型下载等 |
| `2026-06-26` | 集成 LocateAnything-3B 大模检测框自动标注功能                   |
