...
from models import AnnotationTask, ReviewTask, VideoPool, Project, User, ExtractionInfo
        ...
        if request.task_type == 'annotation':
            ...
            if existing_task:
                ...
                if project_name and (existing_task.project_name != project_name):
                    logger.info(f"Updating project_name for task {item_path}: '{existing_task.project_name}' -> '{project_name}'")
        ...
        if task:
            ...
            if task.project_name == 'default' and '/' in request.task_path:
                ...
                if project:
                    ...
                    logger.info(f"Updated project_name from 'default' to '{extracted_project}' for task {request.task_path}")
            ...
            logger.info(f"Task {request.task_path} assigned to {request.new_assignee} (from video_pool) with total_images={total_images}")
