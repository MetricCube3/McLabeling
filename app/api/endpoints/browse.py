        ...
        if os.path.exists(VIDEO_DIR):
            for root, _, filenames in os.walk(VIDEO_DIR):
                for filename in filenames:
                    ...
                    # 如果没有created_at（未分配），使用文件修改时间
                    created_at = task_info.get('created_at', '')
                    if not created_at:
                        file_mtime = os.path.getmtime(video_abs_path)
        ...
        if os.path.exists(IMAGE_DIR):
            for project_dir in os.listdir(IMAGE_DIR):
                ...
                for task_dir in os.listdir(project_path):
                    ...
                    # 如果没有created_at（未分配），使用文件夹修改时间
                    created_at = task_info.get('created_at', '')
                    if not created_at:
                        dir_mtime = os.path.getmtime(task_abs_path)
                        created_at = datetime.fromtimestamp(dir_mtime).isoformat()
...
def browse_annotated(
    ...
    # 如果没有path，列出审核任务
        ...
        if is_admin:
            ...
            # 审核员只能看到分配给自己的任务
            tasks = db.query(ReviewTask).filter(ReviewTask.assignee == user).order_by(
                ReviewTask.created_at.desc()).all()
...
def calculate_task_stats(task_path: str, task_type: str, project_name: str) -> dict:
    ...
    if task_type == 'video':
        ...
        # 如果没有抽帧图片，total_images保持为0（不自动读取视频原始帧数）
        # 用户需要先抽帧后才能看到帧数
